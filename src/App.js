...
class App extends Component {
  ...
  deleteHistory = uniqueNo => {
    ...
    const filterList = historyList.filter(each => each.id !== uniqueNo)
    this.setState({historyList: filterList})
  }

  render() {
    const {userInput, historyList} = this.state
    const searchHistory = historyList.filter(each =>
      each.title.includes(userInput),
    )

    const msg = historyList.filter(each => each.title.includes(userInput)) === 0
